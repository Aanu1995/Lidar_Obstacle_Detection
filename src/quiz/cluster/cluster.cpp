...
pcl::PointCloud<pcl::PointXYZ>::Ptr CreateData(std::vector<std::array<float, 3>> points) {
...
std::vector<std::vector<int>> euclideanCluster(const std::vector<std::array<float, 3>>& points, KdTree* tree, float distanceTol) {
	// This function to return list of indices for each cluster
	...
	for (int i = 0; i < points.size(); i++) {
		...
		// If point is not processed, start a new cluster
	...
	// If no clusters found, return empty vector
	if (clusters.empty()) {
...
int main () {
		...
		// Create data
		std::vector<std::array<float, 3>> points = { {-6.2,7}, {-6.3,8.4}, {-5.2,7.1}, {-5.7,6.3}, {7.2,6.1}, {8.0,5.3}, {7.2,7.1}, {0.2,-7.1}, {1.7,-6.9}, {-1.2,-7.2}, {2.2,-8.9} };
    ...
    for (int i=0; i<points.size(); i++)
  	...
  	int iteration = 0;
  	render2DTree(tree->root,viewer,window, iteration);
